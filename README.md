...
### 🎙️ 语音库存
- 使用系统语音识别录入库存操作
- 默认使用本地规则解析，不需要上传库存数据
- 可扩展为兼容 OpenAI 的 LLM 解析，但 API key 不会打包进 APK；当前默认包没有用户可见的 AI key 设置入口
...
- 示例数据会在首次安装时自动添加
- 数据存储在本地，自动云备份默认关闭，卸载 App 会丢失数据
- 不要把第三方 AI 服务的 API key 打包到 APK；如需真实 LLM smoke test，请只通过 GitHub Actions secret 或本机环境变量提供
