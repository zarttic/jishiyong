...
class InventoryAgentTest {
    ...
    @Test
    fun previewConsumeSelectionExcludesCandidatesWithInsufficientRemainingQuantity() {
        val shortMilk = item(
            id = 1,
            name = "蒙牛纯牛奶 250ml",
            quantity = 2,
            usedQuantity = 1,
            expirationDate = LocalDate.of(2026, 6, 8)
        )
        val enoughMilk = item(
            id = 2,
            name = "伊利纯牛奶",
        ...
        val state = agent.preview("喝了两瓶牛奶", listOf(shortMilk, enoughMilk))

        assertTrue(state is VoiceInputState.PendingConfirmation)
        val pending = state as VoiceInputState.PendingConfirmation
        assertEquals(enoughMilk, pending.matchedItem)
    }
    ...
    @Test
    fun previewConsumeRefusesQuantityGreaterThanRemainingQuantity() {
        val milk = item(id = 1, name = "蒙牛牛奶", quantity = 3, usedQuantity = 1)

        val state = agent.preview("喝了三瓶蒙牛牛奶", listOf(milk))
