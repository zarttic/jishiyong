...
class InventoryCommandParser {
    ...
    private fun parseAdd(text: String, today: LocalDate): InventoryAction {
        ...
        val purchaseDate = parsePurchaseDate(text, today)
    ...
    private fun parseConsume(text: String): InventoryAction {
        val beforeVerb = substringBetweenHandleAndKeyword(text, consumeKeywords)
        val candidate = if (beforeVerb.isNotBlank()) {
            beforeVerb
        } else {
            substringAfterFirstKeyword(text, consumeKeywords)
        }.trimPunctuation()
    ...
    private fun substringAfterFirstKeyword(text: String, keywords: List<String>): String {
        val match = findFirstKeywordMatch(text, keywords)
        return if (match == null) text else text.substring(match.second + match.first.length)
    }

    private fun findFirstKeywordMatch(text: String, keywords: List<String>): Pair<String, Int>? {
        return keywords
            .mapNotNull { keyword ->
    ...
    private fun parseExpirationDate(text: String, today: LocalDate): LocalDate? {
        val context = expirationDateContext(text)
        return parseExpirationDateFromText(context, today)
            ?: if (context == text) null else parseExpirationDateFromText(text, today)
    }

    private fun parseExpirationDateFromText(text: String, today: LocalDate): LocalDate? {
        ...
        return when {
            text.contains("今天过期") || text.contains("今天到期") -> today
            text.contains("明天过期") || text.contains("明天到期") -> today.plusDays(1)
            text.contains("后天过期") || text.contains("后天到期") -> today.plusDays(2)
            else -> null
        }
    }

    private fun expirationDateContext(text: String): String {
        val expirationMatch = findFirstKeywordMatch(text, expirationKeywords) ?: return text
        val lastSeparator = clauseSeparators.maxOf { separator ->
            text.lastIndexOf(separator, startIndex = expirationMatch.second)
        }
        val addEndBeforeExpiration = addKeywords
            .mapNotNull { keyword ->
                val index = text.lastIndexOf(keyword, startIndex = expirationMatch.second)
                if (index >= 0) index + keyword.length else null
            }
            .maxOrNull()
            ?: 0
        val start = maxOf(lastSeparator + 1, addEndBeforeExpiration)
        val end = clauseSeparators
            .mapNotNull { separator ->
                val index = text.indexOf(
                    separator,
                    startIndex = expirationMatch.second + expirationMatch.first.length
                )
    ...
    private fun parsePurchaseDate(text: String, today: LocalDate): LocalDate {
        val addMatch = findFirstKeywordMatch(text, addKeywords) ?: return today
        val beforeAddKeyword = text
            .substring(0, addMatch.second)
            .substringAfterLastClauseSeparator()

        return when {
            beforeAddKeyword.contains("前天") -> today.minusDays(2)
            beforeAddKeyword.contains("昨天") -> today.minusDays(1)
            beforeAddKeyword.contains("明天") -> today.plusDays(1)
            beforeAddKeyword.contains("今天") -> today
            else -> parsePurchaseAbsoluteDate(beforeAddKeyword, today) ?: today
        }
    }

    private fun parsePurchaseAbsoluteDate(text: String, today: LocalDate): LocalDate? {
        yearMonthDayRegex.find(text)?.let { match ->
            val year = match.groupValues[1].toIntOrNull() ?: return null
            val month = parseNumber(match.groupValues[2]) ?: return null
            val day = parseNumber(match.groupValues[3]) ?: return null
        ...
        monthDayRegex.find(text)?.let { match ->
            val month = parseNumber(match.groupValues[1]) ?: return null
            val day = parseNumber(match.groupValues[2]) ?: return null
            return safeDate(today.year, month, day)
        }

        dayOnlyRegex.find(text)?.let { match ->
            val day = parseNumber(match.groupValues[1]) ?: return null
            return safeDate(today.year, today.monthValue, day)
        }
    ...
    private fun String.substringAfterLastClauseSeparator(): String {
        val index = clauseSeparators.maxOf { lastIndexOf(it) }
    ...
    private companion object {
        ...
        private val dayOnlyRegex = Regex("$numberPattern(?:日|号)")
